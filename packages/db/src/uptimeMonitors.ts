...
type UptimeSettingsDbRow = Omit<UptimeSettingsRow, "user_id"> & {
  user_id: string;
};

type UptimeIncidentContextRow = UptimeIncidentRow &
  Omit<UptimeSettingsRow, "user_id"> & {
    user_id: string;
    site_url: string;
...
function toSettingsRow(row: UptimeSettingsDbRow): UptimeSettingsRow {
  return { ...row };
}
...
async function ensureSettingsForSite(
  ...
  const res = await client.query<UptimeSettingsDbRow>(
    `
      SELECT
        s.id,
        s.site_id,
...
function toIncidentError(input: {
  statusCode: number | null;
  errorMessage: string | null;
  errorCode: string | null;
}): string | null {
...
export async function recordUptimeCheck(
  ...
  try {
    const settingsRes = await client.query<UptimeSettingsDbRow>(
      `
        SELECT
          s.id,
          s.site_id,
...
export async function getUptimeStatusForSiteForUser(
  ...
  const [
    ...
    client.query<{ resolved_at: Date | null }>(
      `
        SELECT resolved_at
        FROM uptime_incidents
        WHERE settings_id = $1
...
export async function getUptimeIncidentById(
  ...
  const client = await ensureConnected();
  const res = await client.query<UptimeIncidentContextRow>(
    `
      SELECT
        i.id,
        i.settings_id,
  ...
  return {
    ...
    settings: {
      ...
      created_at: (row as { settings_created_at: Date }).settings_created_at,
      updated_at: (row as { settings_updated_at: Date }).settings_updated_at,
    },
