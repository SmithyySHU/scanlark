...
export interface ReportShareWithToken extends ReportShareRow {
  shareToken: string;
}
...
export async function getReportShareForRunForUser(
  userId: string,
  scanRunId: string,
): Promise<ReportShareWithToken | null> {
...
export async function createOrRotateReportShareForRunForUser(
  ...
  return {
    share: withToken(insertRes.rows[0]),
    created: true,
  };
}
